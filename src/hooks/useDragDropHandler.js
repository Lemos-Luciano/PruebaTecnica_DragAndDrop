...
const useDragDropHandler = (initialItems) => {
	...
	const handleDragDrop = (results) => {
		...
		if (type === 'group') {
			...
			if (
				sourceDroppableID !== 'Header' &&
				destinationDroppableID === 'Header' &&
				nameSelectedItem !== 'image'
			)
				return showAlert(
					'Por suprema orden, solo se permiten agregar imágenes al Header.',
				)

			if (
				sourceDroppableID !== 'Footer' &&
				destinationDroppableID === 'Footer' &&
				nameSelectedItem !== 'text'
			)
				return showAlert(
			...
			const [removeItem] = reorderedStores[sourceDroppableID].splice(
				sourceIndex,
				1,
			)
			reorderedStores[destinationDroppableID].splice(
